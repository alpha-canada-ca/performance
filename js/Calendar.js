...
(function (root, factory) {
  ...
  Calendar.prototype.presetCreate = function() {
    ...
    [{
      ...
      end: moment(self.latest_date).subtract(1, 'day')
    },{
      label: 'Last 6 weeks',
      start: moment(self.latest_date).subtract(6, 'week'),
      end: moment(self.latest_date).subtract(1, 'day')
    },{
      label: 'Last month',
      ...
      label: 'All time',
      start: moment(self.earliest_date),
      end: self.latest_date
    }];
    ...
    if (moment().quarter() == 2) {
      presets.splice(6, 0, {
        ...
        start: moment().subtract(2, 'year').month('April').startOf('month'),
        end: moment().subtract(1, 'year').month('March').endOf('month')    
      });
      presets.splice(6, 0, {
        label: 'Current fiscal year',
        start: moment().subtract(1, 'year').month('April').startOf('month'),
        end: moment().month('March').endOf('month')
      });
    } else {
      presets.splice(6, 0, {
        label: 'Last fiscal year',
